clear;clc;
T = readtable('cylinder_data_2_10.csv');
w=20; %define the angular velocity?
X = T.Points_0(:);
Y = T.Points_1(:);
P = T.p(:);
...
%     if (mod(i-1,2)==1)
%         Fx = Fx - 2*r*P(i)*cos(Theta(i))*(dTheta(i-1)/3);
%         Fy = Fy - 2*r*P(i)*sin(Theta(i))*(dTheta(i-1)/3);
%         Test = Test+2*h/3;
%     else
%         Fx = Fx - 4*r*P(i)*cos(Theta(i))*(dTheta(i-1)/3);
%         Fy = Fy - 4*r*P(i)*sin(Theta(i))*(dTheta(i-1)/3);
%         Test = Test+4*h/3;
%     end
end
